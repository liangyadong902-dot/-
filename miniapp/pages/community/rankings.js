...
Page({
  ...
  onLoad() { this.load() },
  async load() { this.setData({ loading: true, error: '' }); try { const creators = await api.listCreators({ metric: this.data.metric, period: 'all', page: 1, pageSize: 50 }); this.setData({ creators: creators || [], loading: false }) } catch (e) { this.setData({ loading: false, error: '达人榜暂时无法加载' }) } },
