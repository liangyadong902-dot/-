...
| 日期 | 批次 | 状态 | 内容 |
|------|------|------|------|
| 2026-09-19 | 第 2 批 | 进行中 | 账号统计、管理端行程真实 API、管理端交易只读权限收口；支付模块按要求留到最后 |
| 2026-09-20 | 前三阶段收尾 | 代码与自动测试完成，在线写入阻塞 | 微信登录服务、资料编辑、统计、用户隔离、订单前置安全、管理端真实列表与角色权限已覆盖；支付和短信冻结 |
...
### 7.3 非支付、非短信收尾状态（2026-09-20）

- [x] 小程序首页、图鉴、订单、行程、我的不再使用本地盲盒、线路、徽章、订单或行程静态数组。
- [x] 接口失败时清除旧交易缓存并展示错误或空状态，不静默回退到业务假数据。
- [x] 微信登录服务覆盖成功、无效 code、重复 openid 和禁用用户测试。
- [x] `/api/v1/me` 支持昵称、HTTPS 头像、性别和城市修改；本地临时头像不冒充后端永久地址。
- [x] `/api/v1/me/stats` 按有效行程统计；退款失效行程不计入，已解锁徽章独立保留。
- [x] 用户订单与行程查询按当前用户 ID 隔离，越权访问表现为 404。
- [x] 下架盲盒、重复下单、订单快照、15 分钟超时和未支付不生成行程已有自动测试。
- [x] 管理端订单、退款、流水、行程、用户列表读取后端接口；AI、人格和系统设置标记为后续阶段。
- [x] 管理角色权限覆盖：`cs` 可补单、`operator` 不可审核退款、`analyst` 可读和导出订单。
- [ ] 使用微信开发者工具有效 code 完成真实登录落库和双用户手工隔离验收。
- [ ] 服务器账号补齐写权限后，执行改价、下架、超时取消和资料落库的在线验收。
- [ ] 固定 375px 视口完成首页、图鉴、筛选、空态、错误态和弹窗截图归档。

支付与短信在本轮明确冻结：支付宝沙箱支付、notify 验签、原路退款、短信供应商和 Redis 频控不计入上述完成项。
